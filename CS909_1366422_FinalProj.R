...
packs.rqrd <- c("tm", "SnowballC", "RWeka", "slam","plyr","class","topicmodels","lda")
lapply(packs.rqrd, require, character.only = TRUE)

options(stringsAsFactors = FALSE)

# variables in problem

trg.topics <- c("earn","acquisitions","money-fx","grain","crude","trade","interest","ship")

#local functions needed

#Tokenize with Bigrams
mt_bigramtokenizer <- function(clean.corpus) {
NGramTokenizer(clean.corpus, Weka_control(min = 2, max = 2))
}

#Tokenize with Trigrams
mt_trigramtokenizer <- function(clean.corpus) {
	NGramTokenizer(clean.corpus, Weka_control(min = 3, max = 3))
}

mt_createvect <- function(sfilter.vect,clean.corpus) {
	temp.vect <- c()
	q <- 1
	for(q in 1:length(clean.corpus)){
		if (sfilter.vect[[q]] == TRUE) {temp.vect <- append(temp.vect,TRUE)}
		else {temp.vect <- append(temp.vect,FALSE)}
	q <- q + 1
}
	return(temp.vect)
}

mt_sfilter <- function(clean.corpus,trg.top) {
	sfilter.vect <- sFilter(clean.corpus,"Topics == trg.top")
	return(sfilter.vect)
}
...
#Accordingly I will skip the creation of CTM and LDA as these are already functions defined by the package.
